...
if not connection_string:
    ...
    connection_string=":14561"


# Connect to the Vehicle
print('Connecting to follower on: %s' % connection_string)
vehicle = connect(connection_string, wait_ready=True)

leader_connection_string=':14552'
print('Connecting to leader on: %s' % leader_connection_string)
leader_vehicle = connect(leader_connection_string, wait_ready=True)
...
while True:
    targetLocation= get_location_metres(leader_vehicle.location.global_frame,10,0)
    ...
    speed=min(k*distance,10) 
    velocity_x=speed*math.cos(math.pi*bearing/180)
    velocity_y=speed*math.sin(math.pi*bearing/180)
    velocity_z=0

    print(f'Bearing, {bearing:.0f}, Distance: {distance:.1f} Vx: {velocity_x:.1f}, Vy: {velocity_y:.1f}' )
